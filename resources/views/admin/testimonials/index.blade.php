      ...
      <td>
        <div style="display: flex; gap: 12px; align-items: center; justify-content: flex-start;">
          <form method="POST" action="{{ route('admin.testimonials.setstatus', $t->id) }}" style="display:inline-block">
            ...
            <button class="btn btn-lg" style="background:#495057;color:#fff;min-width:70px;font-size:1.1em;padding:8px 20px;">Set</button>
          </form>
          <form method="POST" action="{{ route('admin.testimonials.destroy', $t->id) }}" style="display:inline-block">@csrf @method('DELETE')
            <button class="btn btn-danger btn-lg" style="min-width: 90px; font-size: 1.1em; padding: 8px 20px;" onclick="return confirm('Delete?')">Delete</button>
          </form>
        </div>
      </td>
    </tr>
