...
<script>
<?php 
// Use URL parameters instead of session to avoid persistence issues
$hasSuccess = false;
$hasError = false;
$errorMsg = '';
...
if(isset($_GET['modal'])) {
    ...
    if($modalType === 'success_register') {
        $hasSuccess = true;
    } elseif($modalType === 'success_login') {
        $hasSuccess = true;
    } elseif($modalType === 'error' && isset($_GET['msg'])) {
        $hasError = true;
        $errorMsg = $_GET['msg'];
    }
    
    // Remove the parameter from URL using JavaScript to prevent modal on refresh
